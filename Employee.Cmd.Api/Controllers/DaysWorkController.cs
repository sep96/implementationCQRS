...
namespace Employee.Cmd.Api.Controllers
{
    ...
    {
        private ILogger<DaysWorkController> _logger;
        private ICommandDispatcher _commandDispatcher;

        public DaysWorkController(ILogger<DaysWorkController> logger, ICommandDispatcher commandDispatcher)
        {
            _logger = logger;
            _commandDispatcher = commandDispatcher;
        }
        [HttpPatch("{id}")]
        public virtual async Task<ActionResult> DaysWorkCommandAsync(Guid id, DaysWorkCommand command)
        {
            command.Id = id;
            try
            {
